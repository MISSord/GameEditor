...
- [x] 硬控互斥：新硬控替换旧硬控，或高 Priority 覆盖（在第 1 条 PreReceiveStatus 里做）

**游戏对照：** 眩晕断招、冻结停动画、沉默只能闪避、霸体抬抗打断（高等级打断仍能破）。  
**现状：** `MoveForbid` 沿变进 `PlayerStateEnum.Control`；硬控中跳过 0.35s 受击。冻结 Buff `20601` 推 `Buff.Freeze`，实体钟=0 + 冰壳；点燃仍走世界钟。断招走 `CombatInterrupt`（段表打断 vs `CombatPoiseComponent` 抗打断）。硬控互斥：问 `Buff.MoveForbid`，新硬控 Priority ≥ 已有则 `Replaced` 卸旧；更低则本次不落地。冻结 Priority 20 高于眩晕 10。沉默不进本组。
