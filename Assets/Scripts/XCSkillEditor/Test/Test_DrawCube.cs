...
public class Test_DrawCube : MonoBehaviour
{
    ...
    private void OnDrawGizmos()
    {
        ...
        foreach (var dic in drawDic)
        {
            ...
            {
                if(item.Key.colliderType == ColliderType.Box)
                {
                    DrawCollider(item.Key.pos, item.Key.size, item.Key.rotation, item.Value);
                }
                else if (item.Key.colliderType == ColliderType.Sphere)
                {
                    DrawSphere(item.Key.pos, item.Key.radius, item.Key.rotation, item.Value);
                }
            }
        }
    }
    ...
    void DrawSphere(Vector3 center, float radius, Vector3 rota, Transform targetTran)
    {
        Gizmos.color = drawColor;
