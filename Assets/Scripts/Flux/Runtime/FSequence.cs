...
//		}
		...
		public float Speed { 
			...
			set{ 
				...
				_speed = value; 
				_isPlayingForward = Speed * GameTimeManager.WorldScale >= 0;
   ...
   //   //                      //Debug.Log($"yns  {track.Cache.IsBuilt}"); 
		...
		public void Play( float startTime )
		{
			...
			case AnimatorUpdateMode.Normal:
				_lastUpdateTime = GameTimeManager.WorldTime;
				break;
			case AnimatorUpdateMode.AnimatePhysics:
				_lastUpdateTime = GameTimeManager.FixedWorldTime;
				break;
		...
		protected virtual void Update()
		{
			...
			InternalUpdate( _updateMode == AnimatorUpdateMode.Normal ? GameTimeManager.WorldTime : Time.unscaledTime );
		}
		...
		protected virtual void FixedUpdate()
		{
			...
			InternalUpdate( GameTimeManager.FixedWorldTime );
		}
...
//			UpdateTimelineIds();
		...
		private void OnGUI()
		{
			...
			if( _watermarkLabelRect.width == 0 )
			{
				...
				_watermarkEndTime = Time.unscaledTime + watermarkDuration;
			...
			if( Time.unscaledTime < _watermarkEndTime )
			{
				_watermarkAlpha -= Time.unscaledDeltaTime / watermarkDuration;
