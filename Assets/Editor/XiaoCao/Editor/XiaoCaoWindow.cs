...
namespace XiaoCao
{
    ...
    public class XiaoCaoWindow : EditorWindow
    {
        ...
        private Dictionary<int, List<ButtonAttribute>> _methodDic;
        private List<ButtonAttribute> _noPosMethod;  //无顺序方法放最后
        ...
        public virtual void OnEnable()
        {
            ...
            //获取按钮方法
            _methodDic = new Dictionary<int, List<ButtonAttribute>>();
            _noPosMethod = new List<ButtonAttribute>();
           ...
           .GetMethods(BindingFlags.Instance | BindingFlags.Static | BindingFlags.Public |
            ...
            foreach (var item in _methods)
            {
                ...
                if (att.pos > -10)
                {
                    if (!_methodDic.ContainsKey(att.pos))
                    {
                        _methodDic.Add(att.pos, new List<ButtonAttribute>() { att });
                    }
                    else
                    {
                        _methodDic[att.pos].Add(att);
                    }
                }
                else
                    _noPosMethod.Add(att);
            }
        ...
        void DrawEndBtn()
        {
            foreach (var item in _noPosMethod)
            {
        ...
        {
            if (_methodDic.ContainsKey(index))
            {
                EditorGUILayout.Separator();
                EditorGUILayout.BeginHorizontal();
                foreach (var att in _methodDic[index])
                {
